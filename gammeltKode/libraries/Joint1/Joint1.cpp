...
{
    int offset = 2000;
    int columnFricLSled = 158;//185; //162; //200;
	int columnFricRSled = 221;//258; //230; //270;
    int gammaLSled = 36; //55,47*0.64
	int gammaRSled = 52; //55,47*0.94
    ...
    switch (_outputSEL) {
        case 1:
            if (offset + value <= 0) {
                ...
                if (value < 0) {
                    analogWrite(DAC0,offset + value- columnFricRSled);//+ gammaRSled*(readVel()));
                }
                else if (value > 0){
                    analogWrite(DAC0,offset + value + columnFricLSled);//+ gammaLSled*(readVel()));
                }
                else {
